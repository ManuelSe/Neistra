"""Add atomic-detail visibility defaults without losing retained history.
                ...
                for settings in _settings(name, field, value):
                    for key in FIELDS:
                        if downgrade:
                            if settings.get(key):
                                raise RuntimeError(
                                    f"Cannot downgrade 0012: {name}.{field} "
                                    "retains hidden atom state"
                                )
