"""Add selection surface defaults without losing retained history.
                    ...
                    for key in FIELDS:
                        if downgrade:
                            if settings.get(key) is not None:
                                raise RuntimeError(
                                    f"Cannot downgrade 0011: {name}.{field} "
                                    "retains selection surface state"
                                )
